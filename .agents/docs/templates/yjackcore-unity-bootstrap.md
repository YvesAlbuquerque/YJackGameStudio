...
## 1) `AGENTS.md` — Technology Stack block
...
## 3) `.agents/docs/technical-preferences.md` — Framework Integration defaults
...
- **Framework Routing Notes**: Read `.agents/docs/yjackcore-authority.md` first, then `.agents/docs/yjackcore-support.md`, then `unity-specialist` for generic Unity API behavior.
...
Use `.agents/docs/templates/yjack-workspace.json` and choose one layout:
