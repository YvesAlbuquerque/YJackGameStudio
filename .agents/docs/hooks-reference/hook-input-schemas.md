...
This documents the JSON payloads used by the bundled Claude-style hook scripts.
Other tools can reuse the scripts if they provide equivalent payloads or an
adapter layer.
...
Fired when an agent tool session begins. **No stdin input** — the hook just runs and its stdout is shown to AI agent as context.
...
Fired when the agent tool session ends. **No stdin input** — the hook runs for cleanup and logging.
...
| 2 | Block (stderr shown to AI agent) | PreToolUse only |
